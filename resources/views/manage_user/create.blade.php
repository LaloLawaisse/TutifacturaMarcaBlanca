...
{!! Form::open(['url' => action([\App\Http\Controllers\ManageUserController::class, 'store']), 'method' => 'post', 'id' => 'user_add_form' ]) !!}
  ...
  @component('components.widget')
    <!-- Línea superior: Número + Botón ARCA -->
    <div class="col-md-4">
        <div class="form-group">
            {!! Form::label('document_number', 'Número:*') !!}
            {!! Form::text('document_number', null, ['class' => 'form-control', 'required', 'placeholder' => 'Ingrese el número']); !!}
        </div>
    </div>
    <div class="col-md-2">
        <div class="form-group">
            <label style="visibility:hidden;">Botón</label>
            <button type="button" class="btn" id="btn_buscar_arca"
                style="width: 100%; background-color: #c8e6c9; color: #2e7d32;">
    ...
    <div class="col-md-4">
        <div class="form-group">
            <label style="visibility:hidden;">Botón</label>
            <button type="button" class="btn" id="btn_buscar_arca"
                style="width: 100%; background-color: #c8e6c9; color: #2e7d32; visibility:hidden;">
                <i class="fa fa-search"></i> ARCA
            </button>
        </div>
    </div>
      ...
      <div class="col-md-3">
        <div class="form-group">
          ...
          {!! Form::select('document_type', ['CUIT/CUIL' => 'CUIT/CUIL', 'DNI' => 'DNI'], null, ['class' => 'form-control', 'required']); !!}
        </div>
      </div>

      <div class="col-md-4">
        <div class="form-group">
          {!! Form::label('tax_category', 'Categoría Impositiva:*') !!}
...
<script>
    // Acción botón ARCA
    $('#btn_buscar_arca').on('click', function () {
        const taxId = $('#document_number').val();
        ...
        $.ajax({
            ...
            beforeSend: function () {
                $('#btn_buscar_arca').prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i> Consultando...');
            },
            success: function (response) {
                console.log('Respuesta AFIP:', response);
                ...
                if (response.success) {
                    ...
                    // Seleccionar categoría impositiva por texto
                    if (impuesto) {
                        // Mapeo: descripcionImpuesto (AFIP) => value del select
                        const mapeoValores = {
                        ...
                        const valueSeleccionado = mapeoValores[impuestoNormalizado];
                        
                        if (valueSeleccionado !== undefined) {
                            $('#tax_category').val(valueSeleccionado).trigger('change');
                        } else {
                            console.warn('No se encontró valor para impuesto:', impuesto);
                    ...
                    const direccionCompleta = [domicilio.direccion, domicilio.localidad, domicilio.descripcionProvincia]
                        .filter(Boolean).join(', ');
                    $('#permanent_address').val(direccionCompleta);
                    $('#current_address').val(direccionCompleta);
            ...
            complete: function () {
                $('#btn_buscar_arca').prop('disabled', false).html('<i class="fa fa-search"></i> ARCA');
            }
        });
    });
