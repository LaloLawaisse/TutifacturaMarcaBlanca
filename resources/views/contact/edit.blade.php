  ...
  @endphp
    ...
    <div class="col-md-12">
        <div class="form-group">
            {!! Form::label('cuit_number', 'Número (CUIT):') !!}
            <div class="input-group">
                <span class="input-group-addon">
                    <i class="fa fa-id-card"></i>
                </span>
                {!! Form::text('cuit_number', $contact->tax_number ?? null, ['class' => 'form-control', 'placeholder' => 'Ingrese CUIT', 'id' => 'cuit_number']) !!}
                <span class="input-group-btn">
                    <button type="button" class="btn btn-primary" id="consultar_cuit">Consultar en ARCA</button>
                </span>
            </div>
        </div>
    </div>
      ...
      <div class="row">
        <div class="col-md-6">
            <div class="form-group">
                ...
                {!! Form::text('document_type', $contact->document_type ?? null, ['class' => 'form-control', 'readonly' => true, 'id' => 'document_type']) !!}
            </div>
        </div>
        ...
        <div class="col-md-6">
            <div class="form-group">
                ...
                ], $contact->tax_category ?? null, ['class' => 'form-control select2', 'id' => 'tax_category']) !!}
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.success) {
                ...
                // Llenar razon social o nombre completo
                if (generales.razonSocial && generales.razonSocial.trim() !== '') {
                    $('#supplier_business_name').val(generales.razonSocial);
                } else {
                    const nombreCompleto = [generales.nombre, generales.apellido].filter(Boolean).join(' ');
                    $('#supplier_business_name').val(nombreCompleto);
                }
...
<script>
$(document).ready(function () {
    // Inicializar select2 en campos dinámicos del modal
    $('#tax_category').select2({
        dropdownParent: $('#contact_edit_form').closest('.modal')
    });
    $('#user_id').select2({
        dropdownParent: $('#contact_edit_form').closest('.modal')
    });
    $('#assigned_to_users').select2({
        dropdownParent: $('#contact_edit_form').closest('.modal')
    });
});
</script>
