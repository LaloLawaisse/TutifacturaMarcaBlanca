        ...
        <div class="col-md-12">
           @component('components.filters', ['title' => __('report.filters')])
              {!! Form::open(['url' => '#', 'method' => 'get', 'id' => 'sell_payment_report_form' ]) !!}
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        {!! Form::label('created_by_user_id', __('Creado por') . ':') !!}
                        {!! Form::select(
                            'created_by_user_id',
                            $created_by_users,
                            null,
                            ['class' => 'form-control select2', 'id' => 'created_by_user_id', 'style' => 'width:100%', 'placeholder' => __('messages.all')]
                        ) !!}
                    </div>
                </div>
...
@section('javascript')
  ...
  <script>
  $(function () {
    const $table      = $('#sell_payment_report_table');
    // Asegurate que el ID coincida con el que usa payment.js para inicializar la tabla
    const dt          = $table.DataTable(); // reusa instancia existente
    const $createdBy  = $('#created_by_user_id');

    // Inyectar filtros en cada request
    $table.on('preXhr.dt', function (e, settings, data) {
      // 👉 tu nuevo filtro
      data.created_by_user_id = $createdBy.val() || '';

      // 👉 mantener los demás (usando los IDs reales de tu vista)
      data.supplier_id        = $('#customer_id').val() || '';
      data.location_id        = $('#location_id').val() || '';
      data.payment_types      = $('#payment_types').val() || '';
      data.customer_group_id  = $('#customer_group_filter').val() || '';

      // 👉 rango de fechas si usás daterangepicker en #spr_date_filter
      const drp = $('#spr_date_filter').data('daterangepicker');
      if (drp) {
        data.start_date = drp.startDate.format('YYYY-MM-DD');
        data.end_date   = drp.endDate.format('YYYY-MM-DD');
      } else {
      ...
      // DEBUG
      console.log('[sell_payment_report] filtros enviados:', JSON.parse(JSON.stringify(data)));
    });

    // Recargar al cambiar el filtro (y volver a página 1)
    $createdBy.on('change', function () {
      dt.ajax.reload(null, true);
    });

    // Si tenés botones de aplicar/limpiar filtros:
    $('#apply_filters').on('click', function(){ dt.ajax.reload(null, true); });
    $('#reset_filters').on('click', function(){
      $createdBy.val('').trigger('change'); // limpia y dispara el reload por el handler de arriba
    });
  });
  </script>
@endsection
