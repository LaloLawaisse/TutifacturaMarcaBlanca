...
$(document).ready(function() {
    ...
    $(document).on('click', '.generar-factura', function() {
...
function get_contact_due(id) {
    $.ajax({
        ...
        success: function(res) {
            var valor = (res && res.due) ? String(res.due) : '';
            // Normaliza para que quede "$947.30" (sin espacio)
            valor = valor.replace(/^\$\s+/, '$');

            if (valor) {
                $('.contact_due_text span').text(valor);
                $('.contact_due_text').removeClass('hide');
            } else {
                $('.contact_due_text span').text('');
                $('.contact_due_text').addClass('hide');
            }
        }
    });
