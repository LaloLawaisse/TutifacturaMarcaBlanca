...
class MercadoPagoController extends Controller
{
    ...
    public function obtenerDatosFiscales(Request $request)
    {
        ...
        $accessToken = $user->business->mercadopago_access_token;
        $orders = $request->input('orders', []);
    
        $enrichedOrders = [];
    
        foreach ($orders as $order) {
            $paymentId = $order['main_payment_id'] ?? null;
            ...
            if ($paymentId) {
                try {
                    ...
                    if ($response->ok()) {
                        $data = $response->json();
                        $order['identification'] = $data['payer']['identification'] ?? [];
                        $order['taxpayer_type'] = $data['taxpayer_type'] ?? null;
                    } else {
                        Log::warning("No se pudo obtener datos fiscales para payment_id $paymentId", [
                            'status' => $response->status(),
                            'body' => $response->body(),
                        ]);
                    }
                } catch (\Exception $e) {
                    Log::error("Excepción al obtener datos fiscales: " . $e->getMessage());
                }
            }
