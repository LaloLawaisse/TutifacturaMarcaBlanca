...
class ReportController extends Controller
{
    ...
    public function sellPaymentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionPayment::leftjoin('transactions as t', function ($join) use ($business_id) {
                ...
                ->select(
                    ...
                    // Texto plano (recomendado si no querés rawColumns):
                    DB::raw("
                        IF(
                            ...
                            (
                                SELECT
                                    COALESCE(
                                        NULLIF(CONCAT(TRIM(COALESCE(u2.first_name,'')),' ',TRIM(COALESCE(u2.last_name,''))),' '),
                                        u2.username, ''
                                    )
                                FROM transaction_payments tps
                                JOIN transactions ts ON tps.transaction_id = ts.id
                                LEFT JOIN users u2     ON ts.created_by = u2.id
                    ...
                    // Variante HTML con <br> (si la usás, agrega 'created_by_contact' a rawColumns):
                    // DB::raw("CONCAT(COALESCE(NULLIF(cf.supplier_business_name,''), ''), 
                    //         CASE WHEN NULLIF(cf.supplier_business_name,'') IS NOT NULL THEN '<br>' ELSE '' END,
                    //         COALESCE(cf.name, '')
                    //     ) as created_by_contact"),
            ...
            // Filtro por user creador (created_by)
            if ($request->filled('created_by_user_id')) {
                $uid = (int) $request->get('created_by_user_id');
                $query->where(function ($q) use ($uid, $business_id) {
                    $q->where('transaction_payments.created_by', $uid)
                      ->orWhereRaw("
                          EXISTS (
                              SELECT 1
                              FROM transaction_payments tp
                              JOIN transactions ts ON tp.transaction_id = ts.id
            ...
            // sugerencia: $request->filled('commission_agent')
            ...
            // $dt->rawColumns(['invoice_no', 'amount', 'method', 'action', 'customer', 'created_by_contact']);
        ...
        // ÚNICA asignación (sin duplicar)
        $created_by_users = User::forDropdown($business_id, false);

    
        return view('report.sell_payment_report')
            ->with(compact('business_locations', 'customers', 'payment_types', 'customer_groups', 'created_by_users'));
    }
