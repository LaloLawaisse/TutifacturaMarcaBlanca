...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        // Consultamos el negocio asociado al usuario
        $business = \DB::table('business')
                        ->where('id', $user->business_id)
                        ->first();
    
        // Si existe FacturaTemp, la capturamos y luego la borramos (actualizamos a null)
        if (!empty($business) && !empty($business->FacturaTemp)) {
            $factura_afip = $business->FacturaTemp;
            \DB::table('business')
                ->where('id', $user->business_id)
                ->update(['FacturaTemp' => null]);
        } else {
            $factura_afip = null;
        }
        ...
        $invoice_no = !empty($input['invoice_no'])
            ? $input['invoice_no']
            : $this->getInvoiceNumber($business_id, $input['status'], $input['location_id'], $invoice_scheme_id, $sale_type);
        ...
        // Si los términos de pago están vacíos, se asignan los del contacto.
        ...
        $transaction = Transaction::create([
            ...
            // Asignamos el valor (posible null) de factura_afip obtenido de business.FacturaTemp
            'factura_afip'              => $factura_afip,
            'vencimientoPago' => $input['vencimientoPago'] ?? null,
        ]);
