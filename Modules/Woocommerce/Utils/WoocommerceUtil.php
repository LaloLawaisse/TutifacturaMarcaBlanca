...
class WoocommerceUtil extends Util
{
    ...
    {
        $s = $this->get_api_settings($business_id);
        if (empty($s)) {
            throw new WooCommerceError(__('woocommerce::lang.unable_to_connect'));
        }
    
        // Normalizar URL base: sin /wp-admin, https y minúsculas
        $raw = $s->woocommerce_app_url ?? '';
        $norm = rtrim(preg_replace('#/wp-admin/?$#i', '', $raw), '/');
        // Forzar https y minúsculas (si ya está bien, no cambia)
        $parsed = parse_url($norm);
        $host   = strtolower($parsed['host'] ?? '');
        $scheme = 'https';
        $base_url = $scheme . '://' . $host . (isset($parsed['path']) ? rtrim($parsed['path'], '/') : '');
    
        $ck = $s->woocommerce_consumer_key ?? null;
        $cs = $s->woocommerce_consumer_secret ?? null;
    
        \Log::info('WC woo_client: creando cliente (HTTPS + v3 + BasicAuth)', [
            'business_id' => $business_id,
            'base_url_raw' => $raw,
            'base_url_final' => $base_url,
            'ck_masked' => $ck ? substr($ck,0,6).'...'.substr($ck,-4) : null,
            'cs_masked' => $cs ? substr($cs,0,6).'...'.substr($cs,-4) : null,
            'version' => 'wc/v3',
        ...
        return new \Automattic\WooCommerce\Client(
            ...
            [
                ...
                'version'           => 'wc/v3',
                'timeout'           => 10000,
                'verify_ssl'        => true,          // HTTPS válido
                'query_string_auth' => false,         // ← desactivar OAuth por query
                // 'wp_api_prefix'   => 'index.php?rest_route=/', // solo si /wp-json sigue sin funcionar
            ]
        );
    }
    ...
    {
        \Log::info('WC syncOrders: inicio', [
            'business_id' => $business_id,
            'user_id' => $user_id,
        ]);
    
        $last_synced = $this->getLastSync($business_id, 'orders', false);
        \Log::info('WC syncOrders: last_synced obtenido', [
        ...
        // Contar $orders de forma segura
        $orders_count = (is_array($orders) || $orders instanceof \Countable)
            ? count($orders)
            : (method_exists($orders, 'count') ? $orders->count() : null);
    
        \Log::info('WC syncOrders: órdenes recuperadas de WooCommerce', [
            'orders_count' => $orders_count,
            'tipo_contenedor' => is_object($orders) ? get_class($orders) : gettype($orders),
        ]);
        ...
        \Log::info('WC syncOrders: ventas locales vinculadas encontradas', [
            'woocommerce_sells_count' => $woocommerce_sells->count(),
        ]);
        ...
        $skipped_orders = ! empty($business->woocommerce_skipped_orders)
            ? json_decode($business->woocommerce_skipped_orders, true)
            : [];
    
        \Log::info('WC syncOrders: settings de negocio', [
            'location_id' => optional($woocommerce_api_settings)->location_id,
            'accounting_method' => $business->accounting_method ?? null,
            'skipped_orders_count' => is_array($skipped_orders) ? count($skipped_orders) : 0,
        ]);
        ...
        foreach ($orders as $order) {
            ...
            // ——— DNI por orden/cliente ———
            $dni = $this->extractDniFromOrder($order);
            
            if (empty($dni) && !empty($order->customer_id)) {
                try {
                    $wc = $this->woo_client($business_id);
                    $cust = $wc->get('customers/' . $order->customer_id);
                    $dni = $this->extractDniFromCustomer($cust);
                } catch (\Throwable $e) {
                    \Log::warning('WC DNI: no se pudo leer del customer', [
            ...
            $email = (isset($order->billing) && isset($order->billing->email)) ? $order->billing->email : null;
            $phone = (isset($order->billing) && isset($order->billing->phone)) ? $order->billing->phone : null;
            
            \Log::info('WC orders resumen (DNI)', [
                'order_id'    => $order->id ?? null,
            ...
            // Reglas de skip
            $modificada_despues_ultimo_sync = empty($last_synced) || (strtotime($order->date_modified) > strtotime($last_synced));
            $esta_en_skipped = in_array($order->id, $skipped_orders);
            $es_auto_draft = in_array($order->status, ['auto-draft']);
    
            if ( (!empty($last_synced) && !$modificada_despues_ultimo_sync && !$esta_en_skipped) || $es_auto_draft ) {
                \Log::info('WC syncOrders: saltando orden', $orden_contexto + [
                    'motivo' => $es_auto_draft
                        ? 'auto-draft'
                        : 'no_modificada_desde_ultimo_sync',
                    'last_synced' => $last_synced,
                ]);
                continue;
            }
    
            // Buscar venta local vinculada
            ...
            \Log::info('WC syncOrders: procesando orden', $orden_contexto + [
                'sell_existe' => !empty($sell),
                'sell_status_mapeado' => $sell_status,
                'order_number_final' => $order_number,
            ]);
    
            if (empty($sell)) {
                \Log::info('WC syncOrders: creando venta desde orden', $orden_contexto);
    
                $created = $this->createNewSaleFromOrder($business_id, $user_id, $order, $business_data);
                $created_data[] = $order_number;
    
                if ($created === true) {
                    \Log::info('WC syncOrders: venta creada OK', [
                        'order_number' => $order_number,
                    ]);
                } else {
                    \Log::warning('WC syncOrders: error al crear venta', [
                        'order_number' => $order_number,
                        'error' => $created,
                    ]);
                ...
                \Log::info('WC syncOrders: actualizando venta desde orden', $orden_contexto + [
                    'sell_id' => $sell->id,
                ]);
                ...
                if ($updated === true) {
                    \Log::info('WC syncOrders: venta actualizada OK', [
                        'order_number' => $order_number,
                    ]);
                } else {
                    \Log::warning('WC syncOrders: error al actualizar venta', [
                        'order_number' => $order_number,
                        'error' => $updated,
                    ]);
        ...
        // Resumen y creación de logs de sincronización
        \Log::info('WC syncOrders: resumen preliminar', [
            'creadas_count' => count($created_data),
            'actualizadas_count' => count($updated_data),
            'errores_creacion_count' => count($create_error_data),
            'errores_actualizacion_count' => count($update_error_data),
            'ejemplos_creadas' => array_slice($created_data, 0, 5),
            'ejemplos_actualizadas' => array_slice($updated_data, 0, 5),
        ]);
    
        if (! empty($created_data)) {
            \Log::info('WC syncOrders: creando sync log de creadas');
            $this->createSyncLog($business_id, $user_id, 'orders', 'created', $created_data, $create_error_data);
        }
        if (! empty($updated_data)) {
            \Log::info('WC syncOrders: creando sync log de actualizadas');
        ...
        if (empty($created_data) && empty($updated_data)) {
            ...
            \Log::info('WC syncOrders: sin creadas/actualizadas, creando sync log vacío', [
                'errores_count' => is_array($error_data) ? count($error_data) : 0,
            ]);
            $this->createSyncLog($business_id, $user_id, 'orders', null, [], $error_data);
        }
    
        \Log::info('WC syncOrders: fin');
    }
    
    
    private function flattenMetaValue($v)
    {
        // si viene escalar, lo devuelvo como string
        if (is_null($v)) return null;
        if (is_scalar($v)) return trim((string)$v);
    
        // si viene objeto/array, intento llaves típicas
        if (is_object($v)) $v = (array)$v;
    
        if (is_array($v)) {
            // casos comunes: ['dni'=>'...'], ['number'=>'...'], ['value'=>'...']
            foreach (['dni','documento','document','number','value','vat','vat_number'] as $k) {
    ...
    {
        $wc = $this->woo_client($business_id);
        if ($endpoint === 'orders' && empty($params['status'])) {
            $params['status'] = 'any';
        }
        ...
        \Log::info('WC getAllResponse: inicio', [
            'endpoint' => $endpoint,
        ...
        do {
            $params['page'] = $page;
            try {
                \Log::info('WC getAllResponse: request', [
                    'endpoint' => $endpoint,
                    'page' => $page,
                    'params' => $params,
                ]);
    
                $list = $wc->get($endpoint, $params);
    
                \Log::debug('WC getAllResponse: datos crudos', [
                    'page' => $page,
                    'items' => $list,
                ]);
    
                $resp = method_exists($wc, 'http') ? $wc->http->getResponse() : null;
                \Log::info('WC getAllResponse: response', [
                    'page' => $page,
                    'status' => $resp ? $resp->getCode() : null,
                    'items_en_esta_pagina' => (is_array($list) || $list instanceof \Countable) ? count($list) : null,
                ]);
    
            } catch (HttpClientException $e) {
                $req = $wc->http->getRequest();
                $resp = $wc->http->getResponse();
                \Log::error('WC getAllResponse: HttpClientException', [
                    'message'         => $e->getMessage(),
                    'request_url'     => $req ? $req->getUrl() : null,
                    'request_params'  => $req ? $req->getParameters() : null,
                    'status'          => $resp ? $resp->getCode() : null,
                    'response_body'   => $resp ? mb_substr($resp->getBody(), 0, 4000) : null,
                    'response_headers'=> $resp ? $resp->getHeaders() : null,
                ]);
                return [];
            } catch (\Throwable $e) {
                \Log::error('WC getAllResponse: Exception/Throwable', [
                    'message' => $e->getMessage(),
                ]);
                return [];
            }
    
            $all = array_merge($all, is_array($list) ? $list : []);
            $page++;
        } while (!empty($list));
    
        \Log::info('WC getAllResponse: fin', [
            'endpoint' => $endpoint,
    ...
    public function extractDniFromOrder($o)
    {
        $keys = ['_billing_dni','billing_dni','dni','documento','document',
                 'cuit','cuil','vat','vat_number','billing_vat','billing_vat_number'];
    
        if (!empty($o->meta_data) && is_array($o->meta_data)) {
            foreach ($o->meta_data as $m) {
                $k = is_object($m) ? strtolower((string)($m->key ?? '')) : '';
                $v = is_object($m) ? ($m->value ?? null) : null;
                $v = $this->flattenMetaValue($v);
                if ($k && in_array($k, $keys, true) && !empty($v)) return $v;
            }
        }
    
        if (!empty($o->billing) && is_object($o->billing)) {
            foreach (['dni','document','documento','vat','vat_number'] as $bk) {
                if (isset($o->billing->$bk)) {
                    $v = $this->flattenMetaValue($o->billing->$bk);
                    if (!empty($v)) return $v;
                }
            }
        }
    ...
    public function extractDniFromCustomer($cust)
    {
        $keys = ['billing_dni','dni','documento','document','cuit','cuil','vat','vat_number'];
    
        if (!empty($cust->meta_data) && is_array($cust->meta_data)) {
            foreach ($cust->meta_data as $m) {
                $k = is_object($m) ? strtolower((string)($m->key ?? '')) : '';
                $v = is_object($m) ? ($m->value ?? null) : null;
                $v = $this->flattenMetaValue($v);
                if ($k && in_array($k, $keys, true) && !empty($v)) return $v;
        ...
        if (!empty($cust->billing) && is_object($cust->billing)) {
            foreach (['billing_dni','dni','document','documento','vat','vat_number'] as $bk) {
                if (isset($cust->billing->$bk)) {
                    $v = $this->flattenMetaValue($cust->billing->$bk);
                    if (!empty($v)) return $v;
                }
            }
        }
