...
class SubscriptionController extends BaseController
{
    ...
    public function pay(Request $request, $package_id, $form_register = null)
    {
        ...
        try {
            ...
            if ($package->price == 35000 || $package->price == 50000 || $package->price == 75000 || $package->price == 144000) {
                $gateway = 'automatic_activation';
                $payment_transaction_id = 'MERCADOPAGO';
                $user_id = request()->session()->get('user.id');
            
                $this->_add_subscription(null, $package->price, $business_id, $package, $gateway, $payment_transaction_id, $user_id);
            
                DB::commit();
            
                if (empty($form_register)) {
                    $output = ['success' => 1, 'msg' => 'Paquete activado automáticamente con éxito.'];
            
                    return redirect()
                        ->action([\Modules\Superadmin\Http\Controllers\SubscriptionController::class, 'index'])
                        ->with('status', $output);
                } else {
                    $output = ['success' => 1, 'msg' => 'Registrado y suscrito automáticamente con éxito.'];
    ...
    public function captureMercadoPagoPayment(Request $request)
    {
        try {
            DB::beginTransaction();
    
            // Obtener datos de la notificación o respuesta de MercadoPago
            ...
            if ($response->successful() && $payment_data['status'] === 'approved') {
                $price = $payment_data['transaction_amount'];
                $transaction_id = $payment_data['id'];
    
                // Obtener parámetros necesarios (ajusta según tu lógica)
                $coupon_code = $request->input('coupon_code');
                $package_id = $request->input('package_id');
                $business_id = $request->input('business_id');
                $gateway = 'mercadopago'; // Asegurar que el gateway es correcto
                $user_id = $request->input('user_id');
    ...
    {
        \Log::info('forceActive method called', ['id' => $id]);
    
        $current_date = \Carbon::today();
        $business_id = request()->session()->get('user.business_id');
    
        \Log::debug('Business ID retrieved', ['business_id' => $business_id]);
    
        \DB::beginTransaction(); // Iniciar transacción explícita
        try {
            // Obtener suscripción activa actual
            $active = Subscription::active_subscription($business_id);
            \Log::info('Current active subscription', ['active' => $active]);
    
            if ($active) {
                $new_end_date = $current_date->subDays(1)->toDateString();
                $active->end_date = $new_end_date;
                $active->update();
                \Log::info('Previous subscription deactivated', [
                    'subscription_id' => $active->id,
                    'new_end_date' => $new_end_date
                ]);
            }
    
            // Buscar la suscripción a activar
            $subscription = Subscription::find($id);
            \Log::debug('Subscription to activate', ['subscription' => $subscription]);
    
            if (!$subscription) {
                \Log::error('Subscription not found', ['id' => $id]);
                throw new \Exception(__('lang_v1.subscription_not_found'));
            }
    
            $package = Package::find($subscription->package_id);
            \Log::debug('Package details', ['package' => $package]);
    
            if (!$package) {
                \Log::error('Package not found', ['package_id' => $subscription->package_id]);
                throw new \Exception(__('lang_v1.package_not_found'));
            }
    
            // Calcular nueva fecha de fin
            $end_date = $this->calculate_end_date($package);
            \Log::info('New end date calculated', ['end_date' => $end_date]);
            ...
            \Log::info('Subscription activated successfully', ['subscription_id' => $id]);
            ...
            \DB::rollBack();
            \Log::emergency('Error in forceActive method', [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'message' => $e->getMessage(),
                'trace' => $e->getTraceAsString() // Traza completa
            ]);
